---
title: "Quantium Virtual Internship Project Task 2"
...
```{r Reading the data from the previous task}
filePath = ""
...
```{r Select Control Store}

#### Control these measures over time for each store. 
data [, YEARMONTH := as.integer(format (DATE, "%Y%m"))]


#### Next, we define the measure calculations to use during the analysis.

# For each store and month calculating total sales, number of customers, transactions per customer, chips per customer and the average price per unit.
...
pastCustomers <- measureOverTimeCusts[, Store_type:= ifelse(STORE_NBR == trial_store, "Trial1",
                                                            ifelse (STORE_NBR == control_store, "Control", "Other stores"))
...
ggplot(pastCustomers, aes(TransactionMonth, numberCustomers, color = Store_type)) + 
  geom_line() + 
  labs(x = "Month of Operaton", y = "Total Number of Customers", title = "Total Number of customers by Month")
...
percentageDiff <- merge(scaledControlSales [, c("YEARMONTH", "controlSales")], 
                        ...
                        by = "YEARMONTH")[, percentageDiff := abs(control_store-totSales) / control_store][order(-percentageDiff)]
...
percentageDiff[, tValue := percentageDiff/stdDev][, TransactionMonth := as.Date(paste(YEARMONTH %/% 100, YEARMONTH %% 100, 1, 
                                                     sep = "-"), "%Y-%m-%d")
...
pastSales <- measureOverTimeSales[, Store_type:= ifelse(STORE_NBR == trial_store, "Trial", 
                                 ...
                                 ][, TransactionMonth:= as.Date(paste(YEARMONTH%/%100, YEARMONTH%% 100, 1, sep = "-"), "%Y-%m-%d")][YEARMONTH <210903] 
